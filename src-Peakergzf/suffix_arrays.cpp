...
void AugmentedSuffixArray::all_nf(bool report) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if ((j == n - 1 || lcp[j + 1] <= len) && lcp[j] <= len) {
            if(report)											 	//ADDED BY THOMAS
			  	std::cerr << sa[i] << '\t' << len << '\n'; 				//ADDED BY THOMAS
		   else 													//ADDED BY THOMAS
		   		pos_nf[txt.substr(sa[i], len)]++;
        }
    }
//    if (report) {													//REMOVED BY THOMAS
//        std::ofstream out_file("/dev/null");							//REMOVED BY THOMAS
//        for (const auto &[s, nf]: pos_nf) {							//REMOVED BY THOMAS
//            out_file << s.size() << " " << nf << '\n';					//REMOVED BY THOMAS
//        }														//REMOVED BY THOMAS
//    }															//REMOVED BY THOMAS
...
//            std::cout << "\tpush: ";									//REMOVED BY THOMAS
//             std::cout << lcp[i] << "-[" << lb << ", -]\n";				//REMOVED BY THOMAS
            if (for_next) {
                stk.top().nf++;
                for_next = false;
